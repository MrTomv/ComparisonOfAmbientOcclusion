...
float IntegateArcOfGtao(float h1, float h2, float n, float cosN){
	...
	float h1SinN = h1 * sinN;
	float h2SinN = h2 * sinN;

	float result = 0.25 * (
        (-cosN * h1SinN + h1 * h1SinN + cosN) * acos(-h1) - 
        (-cosN * h2SinN + h2 * h2SinN + cosN) * acos(-h2) +
        sqrt(max(0.0, 1.0 - h1 * h1)) * h1SinN - sqrt(max(0.0, 1.0 - h2 * h2)) * h2SinN);
...
void main(){
	...
	vec2 texSize = 1.0 / Gtao.screenSize;
	...
	for(int i = 0; i < Gtao.directions; i++){
		...
		for(int j = 0; j < Gtao.steps; j++){
			vec2 sUV = TexCoords + direction * PixelRay * float(j + 1) * texSize;
			vec3 sVP = texture(gPosition, sUV).xyz;

			vec3 sDir = sVP - vp;
			float sDist = length(sDir);
			sDir = normalize(sDir);
			float cTheta = dot(normalize(sDir), normal);
			h2 = cTheta;

			if(j > 0 )
				ao += IntegateArcOfGtao(h1, h2, sDist, cTheta);
